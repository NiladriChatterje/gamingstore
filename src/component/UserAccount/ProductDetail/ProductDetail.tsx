...
const ProductDetail = () => {
  ...
  const [_, setPincode] = useState<string>("");
  ...
  async function getProductDetailsFromApi(id: string, pincode: string) {
    const result = await fetch(`http://localhost:5002/fetch-product-detail/${pincode}/${id}`, {
      method: 'GET',
      headers: {
    ...
    if (result.status == 200)
      setSingleProductDetail?.(data as ProductType);
  }

  // Fetch geolocation and get reverse geocode
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      async ({
        ...
        try {
          ...
          const postcode = features[0]?.properties?.postcode;

          if (postcode) {
            setPincode(postcode);
            console.log("Pincode from reverse geocoding:", postcode);
            // Fetch product details with pincode
            getProductDetailsFromApi(id, postcode);
          }
        } catch (error) {
          console.error("Error in reverse geocoding:", error);
          // Fallback: fetch without pincode or with default pincode
          if (id) getProductDetailsFromApi(id, "");
        }
      },
      (error: GeolocationPositionError) => {
        console.error("Geolocation error:", error.message);
        // Fallback: fetch without pincode
        if (id) getProductDetailsFromApi(id, "");
      }
    );
  }, [id])
        ...
        <div id={styles.details__container}>
          ...
          <section id={styles["product-ProductDetail"]}>
            ...
            <section id={styles["product-infos"]}>
              ...
              <div>
                {singleProductDetail && singleProductDetail.quantity &&
                  singleProductDetail.quantity > 0 && (
